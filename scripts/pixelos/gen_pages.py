...
def gen_page(device: str):
    deviceData = {}
    with open(f"_data/devices/pixelos/{device}.yml", 'r') as deviceYaml:
        deviceData = yaml.safe_load(deviceYaml)

    buildData = {}
    try:
        with open(f"ota/pixelos/{device}", 'r') as buildJson:
    ...
    with open(f'_pixelos/{device}.md', 'w') as devicePage:
        devicePage.write('---\n')
        yaml.dump(deviceData, devicePage, sort_keys=False)
        devicePage.write('---')
        devicePage.write('\n\n')
        try:
            with open(f'_instructions/pixelos/{device}.md', 'r') as deviceInstructions:
...
if __name__ == "__main__":
    for i in os.listdir('_data/devices/pixelos'):
